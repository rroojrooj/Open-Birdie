...
//
// CommonJS twin of tools/trace/segment.mjs so the runtime (lib/) can share the
// exact same classifier without an ESM import. tools/trace/segment.mjs is now a
// one-line ESM shim that re-exports these five names.

function ndvi(R, N) { return (N - R) / (N + R + 1e-6); }

// thresholds are the spike's, kept verbatim so behaviour matches the validated spike
function classifyPixel({ R, G, B, N }, texture) {
  const nd = ndvi(R, N);
...
function textureStd(gray, w, h, r = 3) {
...
function segmentWindow(bands, w, h) {
...
const maskOf = (classes, kind) => Uint8Array.from(classes, (c) => (c === kind ? 1 : 0));

module.exports = { ndvi, classifyPixel, textureStd, segmentWindow, maskOf };
