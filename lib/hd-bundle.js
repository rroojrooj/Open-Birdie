...
// Discover the active HD bundle for a course on disk and return a server-owned
// descriptor: an injectable terrain grid (rebased to the course's baseM so it
// blends with the coarse grid), client-safe metadata (no absolute paths, no
// height arrays), and server-only asset paths. Scans the immediate hd-courses
// subdirs (bounded), validates each, and matches by course fingerprint; more than
// one match is rejected as a duplicate rather than guessed.
function resolveHdBundle(course, { dataDir }) {
  ...
  for (const ent of fs.readdirSync(root, { withFileTypes: true }).slice(0, 64)) {
    ...
    let active;
    try { active = JSON.parse(fs.readFileSync(activePath, 'utf8')); } catch { continue; }
    if (typeof active.bundle !== 'string') continue;
    let bundleDir;
    try { bundleDir = resolveAssetPath(courseDir, active.bundle); } catch { continue; }
    const res = validateBundleDirectory(bundleDir);
    if (res.status !== 'valid' || res.descriptor.course.fingerprint !== fingerprint) continue;
    matches.push({ bundleDir, descriptor: res.descriptor });
  }
  if (matches.length === 0) return { status: 'absent' };
  if (matches.length > 1) {
    return { status: 'rejected', code: 'HD_DUPLICATE', message: `${matches.length} HD bundles match this course` };
  }

  const { bundleDir, descriptor } = matches[0];
  ...
  const metadata = {
    bundleId: descriptor.manifestSha256,
    hole: manifest.hole,
    bounds: manifest.bounds,
