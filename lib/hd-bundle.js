...
const fingerprints = require('./course-fingerprint');
...
const TYPED_SELECTION_REJECTIONS = new Set([
...
// Runtime and ESM compiler tooling delegate to lib/course-fingerprint.js; parity
// is cross-checked by test/hd-bundle.test.js.
// ---------------------------------------------------------------------------
function courseFingerprint(course) {
  return fingerprints.courseFingerprintV1(course);
}
...
function validateManifest(m) {
  ...
  const fingerprintVersion = m.course.fingerprintVersion ?? 1;
  if (fingerprintVersion !== 1 && fingerprintVersion !== 2) {
    throw rejectErr(
      'HD_FINGERPRINT_VERSION_UNSUPPORTED',
      `unsupported course fingerprintVersion ${fingerprintVersion}`,
    );
  }
  if (fingerprintVersion === 2) {
    try {
      fingerprints.parseCourseId(m.course.courseId);
    } catch {
      throw rejectErr('HD_SOURCE_ID_REQUIRED', 'v2 bundle manifest requires a valid courseId');
    }
  }
...
function resolveHdBundles(course, { dataDir }) {
  ...
  for (const ent of fs.readdirSync(root, { withFileTypes: true }).slice(0, 64)) {
    ...
    for (const be of fs.readdirSync(bundlesDir, { withFileTypes: true }).slice(0, 64)) {
      ...
      if (res.status === 'rejected') {
        if (TYPED_SELECTION_REJECTIONS.has(res.code)) typedRejection ??= res;
        continue;
      }
      if (res.status !== 'valid') continue;
      const manifestCourse = res.descriptor.course;
      const fingerprintVersion = manifestCourse.fingerprintVersion ?? 1;
      let expected;
      try {
        expected = fingerprints.courseFingerprintFor(course, {
          version: fingerprintVersion,
          courseId: manifestCourse.courseId,
        });
      } catch (error) {
        if (error?.code === 'HD_SOURCE_ID_REQUIRED' || error?.code === 'HD_SOURCE_ID_MISMATCH' ||
            error?.code === 'HD_FINGERPRINT_VERSION_UNSUPPORTED') {
          typedRejection ??= { status: 'rejected', code: error.code, message: error.message };
          continue;
        }
        throw error;
      }
      if (manifestCourse.fingerprint !== expected) continue;
      let mtimeMs = 0;
  ...
  if (candidates.length === 0) return typedRejection || { status: 'absent' };
...
module.exports = {
  ...
  SURFACE_CHANNELS,
  courseFingerprint,
  courseFingerprintFor: fingerprints.courseFingerprintFor,
  courseFingerprintV1: fingerprints.courseFingerprintV1,
  courseFingerprintV2: fingerprints.courseFingerprintV2,
  validateManifest,
  decodeTerrainF32,
