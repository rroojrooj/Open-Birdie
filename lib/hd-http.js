...
// Route an HD asset request to one of N active bundles by content-addressed id.
// Returns the matching descriptor or null (404). The id is compared, never used
// to build a path, so there is no traversal surface.
function pickDescriptor(descriptors, bundleId) {
  if (!Array.isArray(descriptors)) return null;
  return descriptors.find((d) => d && d.bundleId === bundleId) || null;
}

module.exports = { serveHdAsset, publicHdMetadata, pickDescriptor, ASSET_KEYS, MIME };
