...
async function loadCourse({ name, bbox, osmType, osmId }) {
  ...
  if (fs.existsSync(cacheFile)) {
    try {
      ...
      if (cached.version === CACHE_VERSION && cached.holes && cached.holes.length) return cached;
    } catch (_) { /* corrupt/truncated cache — fall through and re-fetch */ }
  }
  ...
  try {
    ...
    if (aer) {
      const aname = slug(name) + '.aerial.jpg';
      fs.writeFileSync(path.join(CACHE_DIR, aname), aer.buf);
  ...
  fs.writeFileSync(tmp, JSON.stringify(course));
...
async function maybeClassify(course, name, {
  ...
  writeFile = (p, buf) => fs.writeFileSync(p, buf),
} = {}) {
  ...
  try {
    ...
    const cname = slug(name) + '.classmap.png';
...
function listCached() {
  ...
  return fs.readdirSync(CACHE_DIR).filter((f) => f.endsWith('.json')).map((f) => {
    ...
    try { return { file: f, name: JSON.parse(fs.readFileSync(fp, 'utf8')).name }; }
    catch (_) {
