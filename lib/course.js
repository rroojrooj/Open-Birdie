...
function migrateLegacyCourseCache({
  ...
  try {
    ...
    for (const entry of pending) fsImpl.renameSync(entry.temporary, entry.finalPath);
...
function createCourseAcquisitionCoordinator(acquireCourse) {
  ...
  return {
    acquire(request, { abortDifferent = false } = {}) {
      const source = normalizeCourseSource(request.source || request);
      ...
      let acquired;
      try {
        acquired = acquireCourse(request, { source, signal: controller.signal });
      } catch (error) {
...
async function loadCourseUncoordinated(
  { name, bbox, lat, lon, osmType, osmId },
  { source = normalizeCourseSource({ osmType, osmId }) } = {},
) {
  fs.mkdirSync(CACHE_DIR, { recursive: true });
  const stem = courseCacheStem(source);
  const cacheFile = path.join(CACHE_DIR, `${stem}.json`);
  ...
  try {
    const aer = await fetchCourseAerial({ origin: course.origin, bounds: courseBounds(course) });
    if (aer) {
      const aname = `${stem}.aerial.jpg`;
      atomicWriteOwned(path.join(CACHE_DIR, aname), aer.buf);
      course.aerial = { file: aname, bounds: aer.bounds };
      console.log(`[aerial] ${aname} (${(aer.buf.length / 1024) | 0} KB)`);
    }
  } catch (e) { console.error('[aerial] skipped:', e.message); }
  ...
  // Atomic write: a process kill mid-write (Electron quit, crash) must not leave
  // a truncated JSON that bricks startup auto-load. Write to a temp file + rename.
  atomicWriteOwned(cacheFile, JSON.stringify(course));
  return course;
}

const courseAcquisition = createCourseAcquisitionCoordinator(loadCourseUncoordinated);

function loadCourse(request) {
  return courseAcquisition.acquire(request);
}
...
async function maybeClassify(course, name, {
  ...
  try {
    const nb = await fetchBands({ origin: course.origin, bounds: courseBounds(course) });
  ...
  } catch (e) { console.error('[classify] skipped:', e.message); }
}
...
// ---- Per-course surface override (data/courses/<slug>.surfaces.json) --------
...
function loadSurfaceOverride(course, dir = CACHE_DIR, curatedDir = CURATED_DIR) {
  ...
  const name = slug(course.name) + '.surfaces.json';
  // Machine-local override wins; fall back to the committed curated fixture so a
  // reconstructed course's surfaces travel with the branch. A present-but-corrupt
  // file surfaces the error (returns null) rather than silently falling through.
  for (const fp of [path.join(dir, name), path.join(curatedDir, name)]) {
...
module.exports = {
  ...
  slug, applySurfaceOverride, loadSurfaceOverride, maybeClassify,
  CourseCacheError, atomicWriteOwned, createCourseAcquisitionCoordinator,
  migrateLegacyCourseCache, ownedTempPath, readSourceKeyedCache,
};
