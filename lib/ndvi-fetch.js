...
async function fetchPng(url, fetchImpl) {
  let buf;
  try {
...
async function fetchNdviBands({ origin, bounds, fetchImpl = fetch, maxPx = NDVI_MAXPX } = {}) {
  ...
  const [rgb, nir] = await Promise.all([
    fetchPng(exportUrl(w, s, e, n, W, H, '0,1,2'), fetchImpl),
    fetchPng(exportUrl(w, s, e, n, W, H, '3,0,1'), fetchImpl),
  ]);
