...
async function fetchPatch({ west, south, east, north }, { targetM = 1, maxPx = 600 } = {}) {
  try {
    ...
    const meta = await (await fetch(q('f=json'), { headers: { 'User-Agent': UA }, signal: AbortSignal.timeout(REQ_TIMEOUT) })).json();
    if (!meta || !meta.width || !meta.height || !meta.extent) return null;
    w = meta.width; h = meta.height;
    const e = meta.extent;

    // 2) raw float32 raster (bsq single band = flat row-major float32)
    const res = await fetch(q('f=image'), { headers: { 'User-Agent': UA }, signal: AbortSignal.timeout(REQ_TIMEOUT) });
