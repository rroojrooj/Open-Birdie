 ...
 */
async function fetchPatchStrict({ west, south, east, north }, { targetM = 1, maxPx = 600, fetchImpl = fetch, timeoutMs = REQ_TIMEOUT } = {}) {
  ...
  let meta;
  try {
    const r = await fetchImpl(q('f=json'), { headers: { 'User-Agent': UA }, signal: AbortSignal.timeout(timeoutMs) });
  ...
  let buf;
  try {
    const res = await fetchImpl(q('f=image'), { headers: { 'User-Agent': UA }, signal: AbortSignal.timeout(timeoutMs) });
