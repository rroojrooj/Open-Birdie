'use strict';
const { isAbortError, throwIfAborted, withTimeoutSignal } = require('./abort');
 ...
 */
async function fetchPatchStrict(
  { west, south, east, north },
  { targetM = 1, maxPx = 600, fetchImpl = fetch, signal } = {},
) {
  ...
  let meta;
  try {
    const r = await fetchImpl(q('f=json'), {
      headers: { 'User-Agent': UA },
      signal: withTimeoutSignal(signal, REQ_TIMEOUT),
    });
  ...
  let buf;
  try {
    const res = await fetchImpl(q('f=image'), {
      headers: { 'User-Agent': UA },
      signal: withTimeoutSignal(signal, REQ_TIMEOUT),
    });
    ...
    if (isAbortError(er, signal)) throw er;
