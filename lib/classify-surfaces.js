...
function classifyToClassmap({ bands, width, height, bounds, boundary, surfaces } = {}) {
  ...
  // Clip to the SAME inside-course mask used for the S2 stats: outside the course
  // (padded-bbox beach / parking / bare ground) R and B are forced 0 so the renderer
  // leaves the real-place far-field aerial untouched there.
  const png = new PNG({ width, height });
  for (let i = 0; i < classes.length; i++) {
    const o = i * 4;
    const c = classes[i];
    const on = inCourse[i]; // off-course pixels emit no mown/sand
    png.data[o] = on && c === 'fairway' ? 255 : 0;
    png.data[o + 1] = 0;
    png.data[o + 2] = on && c === 'sand' ? 255 : 0;
    png.data[o + 3] = 255;
  }
