...
function classifyToClassmap({ bands, width, height, bounds, boundary, surfaces } = {}) {
  ...
  for (let py = 0; py < height; py++) {
    for (let px = 0; px < width; px++) {
      ...
      for (const s of mownPolys) {
        if (pointInPoly(mx, my, s.poly)) { mownMask[i] = 1; break; }
      }
      ...
      if (hasBoundary) {
        ...
        let inside = 0;
        for (const s of allPolys) {
          if (pointInPoly(mx, my, s.poly) || distToPolyEdges(mx, my, s.poly) < DILATE_M) {
            inside = 1; break;
          }
        }
        inCourse[i] = inside;
