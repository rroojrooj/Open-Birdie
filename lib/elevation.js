 ...
 */
async function fetchElevationGrid({ lat0, lon0, mPerLat, mPerLon, minX, minY, maxX, maxY }, { fetchImpl } = {}) {
  ...
  try {
    ...
    const fine = await fetch3depBase({
      ...
      cellM: baseCell, baseM, baseH: gridSampler(coarse).h, fetchImpl,
    });
    if (fine) return { ...fine, baseM: baseMr };
  } catch (_) { /* keep the coarse terrarium base */ }
  return coarse;
}
 ...
 */
async function fetch3depBase({ lat0, lon0, mPerLat, mPerLon, minX, minY, maxX, maxY, cellM, baseM, baseH, fetchImpl }) {
  ...
  }, { targetM: cellM, maxPx: BASE_MAXPX, fetchImpl })));
