...
const TILE = 256;
const GRID_CELL_M = 5;      // coarse-fallback grid spacing; widened if course is huge
const MAX_NODES = 768;      // per axis (coarse fallback)
const PAD_M = 80;
const SMOOTH_SIGMA = 2;     // cells (~10 m) — irons out SRTM stair-steps (coarse only)

// Course-wide 1 m base from USGS 3DEP lidar (US). The whole ground gets real
// relief — bunker pits, green bowls, mounds, swales — not just per-hole patches.
// Per-node fallback to the coarse terrarium grid where 3DEP has no data (water /
// off-US). The 1 m grid is NOT gaussian-smoothed (that would erase the relief).
const BASE_CELL_M = 1;        // target base resolution where 3DEP covers
const BASE_MAX_NODES = 2048;  // perf ceiling/axis (true 1 m up to ~2 km; coarser beyond)
const BASE_TILE_M = 1400;     // course bbox split into <=1400 m 3DEP requests
const BASE_MAXPX = 1500;      // px cap/request (1400 m @ 1 m = 1400 px, under cap)
...
async function fetchTile(z, x, y, { fetchImpl = fetch, signal } = {}) {
  throwIfAborted(signal);
  const res = await fetchImpl(TILE_URL(z, x, y), {
    headers: { 'User-Agent': 'Open-Birdie/0.1' },
    signal,
  });
...
async function fetchElevationGrid(
  { lat0, lon0, mPerLat, mPerLon, minX, minY, maxX, maxY },
  ...
  for (let ty = tyMin; ty <= tyMax; ty++) {
    for (let tx = txMin; tx <= txMax; tx++) {
      jobs.push(fetchTile(ZOOM, tx, ty, { fetchImpl, signal }).then((png) => {
  ...
  const baseMr = Math.round(baseM * 10) / 10;
  const coarse = { minX, minY, cellM, nx, ny, baseM: baseMr, heights: rel };

  // Prefer a course-wide 1 m 3DEP base (US). Per-node it falls back to the coarse
  // grid above (water / off-US), so this never regresses non-US courses. Any
  // failure keeps the coarse base — 1 m relief is an enhancement, never a hard dep.
  try {
    let baseCell = BASE_CELL_M;
    while ((maxX - minX) / baseCell + 1 > BASE_MAX_NODES || (maxY - minY) / baseCell + 1 > BASE_MAX_NODES) baseCell *= 2;
    const fine = await fetch3depBase({
      lat0, lon0, mPerLat, mPerLon, minX, minY, maxX, maxY,
      cellM: baseCell, baseM, baseH: gridSampler(coarse).h, fetchImpl, signal,
    });
    if (fine) return { ...fine, baseM: baseMr };
  } catch (error) {
    if (isAbortError(error, signal)) throw error;
    /* keep the coarse terrarium base */
  }
  throwIfAborted(signal);
  return coarse;
}
...
function makeTerrain(grid, patches = [], { smooth = false } = {}) {
  ...
  // Per patch: a RAW sampler drives h() so rendered and physics ground heights are
  // identical; a (when smooth) SMOOTHED sampler drives grad() so sub-metre scan
  // noise can't jitter putts. `edgeBlendM` overrides the default feather; 0 means
  // the compiler already baked the edge ring — use the patch with NO runtime
  // feather and short-circuit the weight to 1 (avoids 0/0 = NaN at inset === 0,
  // which would otherwise produce NaN vertex heights that detonate the mesh).
  const entries = patches.map((p) => ({
    raw: gridSampler(p),
    grad: gridSampler(smooth ? boxBlur(p) : p),
    edgeBlendM: p.edgeBlendM,
  }));
  const lerp = (a, b, w) => a + (b - a) * w;
  const weightOf = (e, x, y) => {
    const blend = e.edgeBlendM ?? FEATHER_M;
    if (blend <= 0) return 1;
    return Math.min(1, e.raw.inset(x, y) / blend);
  };
  const pick = (x, y) => {
    for (const e of entries) if (e.raw.contains(x, y)) return { e, w: weightOf(e, x, y) };
    return null;
  };
  return {
    flat: false,
    h(x, y) {
      const sel = pick(x, y);
      return sel ? lerp(baseS.h(x, y), sel.e.raw.h(x, y), sel.w) : baseS.h(x, y);
    },
    grad(x, y) {
      const sel = pick(x, y);
      if (!sel) return baseS.grad(x, y);
      const b = baseS.grad(x, y), g = sel.e.grad.grad(x, y);
...
// Split a local-meter bbox into <= tileM tiles (clamped to the bbox edge), one per
// 3DEP request. Pure.
function planLidarTiles(minX, minY, maxX, maxY, tileM) {
  const tiles = [];
  for (let y = minY; y < maxY - 1e-9; y += tileM) {
    for (let x = minX; x < maxX - 1e-9; x += tileM) {
      tiles.push({ minX: x, minY: y, maxX: Math.min(x + tileM, maxX), maxY: Math.min(y + tileM, maxY) });
    }
  }
  return tiles;
}

// Combine mercator samplers: the first that has data (non-null) wins. Pure.
function combineSamplers(samplers) {
  return (mx, my) => {
    for (const s of samplers) { const v = s(mx, my); if (v != null) return v; }
    return null;
  };
}

/**
 * Course-wide 1 m base from tiled 3DEP. Fetches each tile best-effort, combines
 * the samplers, and resamples to the local-meter grid with `baseH` (the coarse
 * terrarium grid) filling any NoData / off-US node. Returns null if NO tile
 * fetched, so the caller keeps the coarse base. Heights are relative to baseM,
 * exactly like the base grid. `fetchImpl` is injectable for offline tests.
 */
async function fetch3depBase({
  lat0,
  lon0,
  mPerLat,
  mPerLon,
  ...
  throwIfAborted(signal);
  const tiles = planLidarTiles(minX, minY, maxX, maxY, BASE_TILE_M);
  const patches = await Promise.all(tiles.map((t) => lidar.fetchPatch({
    west: lon0 + t.minX / mPerLon, east: lon0 + t.maxX / mPerLon,
    south: lat0 + t.minY / mPerLat, north: lat0 + t.maxY / mPerLat,
  }, { targetM: cellM, maxPx: BASE_MAXPX, fetchImpl, signal })));
  throwIfAborted(signal);
  const samplers = patches.filter(Boolean).map((p) => lidar.makePatchSampler(p));
  if (!samplers.length) return null;
  return resampleToLocal({
    sampler: combineSamplers(samplers), minX, minY, maxX, maxY, cellM,
    lat0, lon0, mPerLat, mPerLon, baseM, baseH,
  });
}
...
async function fetchGreenPatches({ lat0, lon0, mPerLat, mPerLon, baseM, greens, baseH },
  ...
  async function worker() {
    while (next < list.length) {
      ...
      }, { targetM: cellM, fetchImpl, signal });
      if (patch) {
...
module.exports = { fetchElevationGrid, makeTerrain, flatTerrain, resampleToLocal, fetchGreenPatches, gridSampler, planLidarTiles, combineSamplers, fetch3depBase };
