...
function loadRuntimeCourseArt({
  ...
  const selected = index.packs.find((entry) => entry.courseId === courseId) ||
  ...
  try {
    ...
    for (const asset of manifest.assets) {
      ...
      const inspected = inspectRuntimeAsset(assetPath, asset);
      totalBytes += inspected.bytes;
    ...
    return { status: 'valid', runtimePack: Object.freeze(runtimePack), diagnostics: [] };
  } catch (error) {
