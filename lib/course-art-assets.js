...
function readVerifiedRuntimeAsset(filePath, declaration) {
  ...
  const inspected = inspectRuntimeAssetBytes(realPath, bytes, declaration);
  ...
  return Object.freeze({
    absolutePath,
    realPath,
    validatedMime: declaration.mime,
...
function loadRuntimeCourseArt({
  ...
  try {
    ...
    for (const asset of manifest.assets) {
      ...
      validatedAssets[asset.key] = verified;
    }
    if (totalBytes > LIMITS.MAX_TOTAL_ASSET_BYTES) {
    ...
    Object.defineProperty(runtimePack, 'validatedAssets', {
      value: Object.freeze(validatedAssets),
      enumerable: false,
    });
    return {
      status: 'valid',
