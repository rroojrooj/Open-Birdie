...
const { inspectRuntimeAssetBytes } = require('./course-art-assets');
...
function metadataMatches(publicEntry, privateEntry, contentRevision, assetKey) {
  return publicEntry && privateEntry &&
    ...
    privateEntry.validatedMime === privateEntry.mime &&
    privateEntry.validatedSha256 === privateEntry.sha256;
}
...
async function readExactHandleBytes(handle, expectedBytes) {
  const bytes = Buffer.allocUnsafe(expectedBytes);
  const result = await handle.read(bytes, 0, expectedBytes, 0);
  if (!result || result.bytesRead !== expectedBytes) {
    throw new Error('opened asset ended early');
  }
  return bytes;
}
...
async function serveCourseArtRequest(req, res, {
  ...
  try {
    ...
    if (!isInside(rootReal, currentReal)) throw new Error('runtime path escaped its root');
    ...
    const requestBytes = await readExactHandleBytes(
      handle,
      selected.privateEntry.bytes,
    );
    const inspected = inspectRuntimeAssetBytes(
      currentReal,
      requestBytes,
      { mime: selected.privateEntry.mime },
    );
    if (inspected.bytes !== selected.privateEntry.bytes ||
        inspected.sha256 !== selected.privateEntry.sha256) {
      throw new Error('opened asset bytes changed');
    }
    ...
    res.statusCode = 200;
    res.end(requestBytes);
  } catch (error) {
    if (typeof onInternalError === 'function') {
      try {
