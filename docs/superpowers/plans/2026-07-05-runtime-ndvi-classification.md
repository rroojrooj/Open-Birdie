...
> ### ✅ Task 0 RESULT — GO (spike run 2026-07-05, proven against the live endpoint)
> **NIR is retrievable with zero new deps and cleanly separates vegetation from sand** (Chambers fairway NDVI **0.31** vs sand **−0.12**, disjoint pixel populations). Load-bearing findings that AMEND Tasks 1–2:
> - **TWO co-registered requests, not one** (band 4 is dropped from any single PNG render; alpha is constant 255, NOT NIR):
>   - RGB: `…/USGSNAIPPlus/ImageServer/exportImage?bbox={w},{s},{e},{n}&bboxSR=4326&imageSR=4326&size={W},{H}&bandIds=0,1,2&format=png&f=image` → pngjs ch0=R, ch1=G, ch2=B.
>   - NIR: same but `&bandIds=3,0,1` → pngjs **ch0=NIR**, ch1=R, ch2=G. Feed `segmentWindow`'s `[R,G,B,NIR]` from RGB(ch0-2)+NIR(ch0). The two are pixel-exact co-registered (identical bbox/size/SR → same grid, per-pixel Δ=0 verified).
> - **`format=png` is MANDATORY — never `jpgpng`** (the existing `lib/aerial.js` uses `jpgpng`, which returns lossy JPEG that pngjs can't decode and that corrupts NIR). This is a NEW, separate PNG fetch, not a reuse of the aerial JPEG.
> - **Calibration (real `segmentWindow`, inside-course mask):** chambers sand **14.6%** / mown **22.8%**; sawgrass sand **14.8%** / mown **7.4%**. `boundary` can be **null** (Sawgrass) → use the **dilated union of surface polygons (25 m)** as the inside-course mask, not a hull.
> - **S2 REDESIGN (supersedes the guessed 0.35/0.80):** the most likely real failure — NIR fetch silently degrades to R — leaves sand% *plausible* (~18%) but collapses **mown% to 0**. A sand-only abort MISSES it. So **PRIMARY guard = mown-floor: abort if `mownPct < 0.03`** (real min 7.4%, 2.5× margin); **SECONDARY = abort if `sandPct > 0.55`** (only needs to catch a ~100%-sand nodata tile; real max ~18%, so a legit sandy links course never trips). Do NOT use the `×1.4` sand formula — it false-trips.
> - **Gotchas:** never trust a single centroid pixel (JSON polys are a few m misregistered vs this NAIP capture → centroid probes can invert; use polygon-interior medians / populations); the `water` class over-fires (`nd<0.02` catches shoreline AND shadow — don't use it as hydrology); classifier thresholds are Chambers-fescue-tuned so parkland mown reads conservative (fine — the 3% floor has margin); no rate-limiting on 2 fetches/load.
...
Mirror `lib/aerial.js` shape (same bbox math, `fetchImpl` injectable for headless test, best-effort → null). **Per the Task-0 result: TWO `format=png` requests** — RGB (`bandIds=0,1,2`) and NIR (`bandIds=3,0,1`) — decoded with `pngjs` and combined into one interleaved `[R,G,B,NIR,…]` buffer (`segmentWindow`'s input). RGB gives R/G/B (ch0-2); NIR gives NIR from ch0. They are pixel-exact co-registered (same bbox/size/SR). **`format=png` is mandatory** — `jpgpng` returns JPEG (pngjs can't decode; lossy corrupts NIR). If EITHER fetch fails → null (best-effort → OSM-only).
...
- [ ] **Step 1: Failing test** (CommonJS, mirror `test/aerial.test.js`): assert BOTH request URLs carry `format=png` (NOT jpgpng), their respective `bandIds` (`0,1,2` and `3,0,1`), the course bbox, `imageSR=4326`, and `size` capped at `NDVI_MAXPX`; assert null if EITHER fetch is non-ok / tiny-body / throws. **Real-PNG decode test (eng-review finding 10):** a `fakeJpeg()` buffer will NOT exercise the pngjs decode — generate two tiny real 3-band PNGs (8×8 via `PNG.sync.write` in the test setup, one standing in for RGB, one for the NIR band-swap) and assert the combined output is `{ bands:[R,G,B,NIR,…], width, height }` with **NIR sourced from the NIR-fetch ch0** and R/G/B from the RGB-fetch ch0-2. The channel split is the part most likely to be wrong; assert it on real bytes.
- [ ] **Step 2:** Run → FAIL. **Step 3:** Implement `fetchNdviBands({ origin, bounds, fetchImpl = fetch, maxPx = NDVI_MAXPX })` → two `format=png` fetches → `PNG.sync.read` each (sync is fine at ≤768px) → interleave `[R,G,B,NIR]`. Guard each body like `lib/aerial.js` (PNG magic bytes `89 50`, 2 KB floor). **Step 4:** Run → PASS. **Step 5:** commit.
...
4. **S2 (Task-0 calibrated design — supersedes the guessed 0.35/0.80):** compute `sandPct`/`mownPct` over pixels **inside the course mask** = the **dilated (25 m) union of surface polygons** (NOT `boundary` — it is `null` on Sawgrass; NOT the padded window — sea/parking inflate sand). **PRIMARY guard: abort if `mownPct < 0.03`** — the real failure mode (NIR silently degrades to R) collapses mown to 0 while leaving sand plausible, so the mown-floor is the true capture-integrity gate (real min 7.4%, 2.5× margin). **SECONDARY: abort if `sandPct > 0.55`** — only to catch a ~100%-sand nodata tile (real max ~18%, so a legit sandy links course never trips). → on abort return `{ classmap: null, stats, aborted: true }`;
5. encode RGBA (R = 255·mown-detected, B = 255·sand-detected, G/A = 0) to a PNG buffer via pngjs;
6. return `{ pngBuffer, stats }`.

- [ ] **Step 1: Failing tests:** (a) a synthetic sand pixel INSIDE an OSM mown poly → sand channel 0 (S1); (b1) **S2 primary** — a window whose in-mask `mownPct < 3%` (the NIR-degraded-to-R failure) → `aborted:true`, null classmap; (b2) **S2 secondary** — `sandPct > 55%` (nodata tile) → `aborted:true`; (b3) a realistic window (mown ~20%, sand ~15% — the Chambers/Sawgrass calibration range) → NOT aborted; (c) a mown-NDVI pixel outside OSM polys → mown channel 255 (the coverage-add that fixes sparse stripes); (d) encoded PNG round-trips to the expected channels; (e) pixels OUTSIDE the dilated surface-union mask are excluded from both percentages.
